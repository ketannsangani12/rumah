            ...
            if (!empty($_POST)) {
                ...
                if ($model->validate()) {
                    ...
                    if (!empty($favouritexists)) {
                        return array('status' => 0, 'message' => 'You already added this property as favourite');
                        exit;
                    }
                    $model->created_at = date('Y-m-d h:i:s');
                    $model->save();
    ...
    public function actionAddproperty()
    {
        ...
        if ($method != 'POST') {
            ...
            if (!empty($_POST)) {
                ...
                $propertymodel->amenities = (!empty($propertymodel->amenities)) ? implode(',', $propertymodel->amenities) : NULL;
                $propertymodel->commute = (!empty($propertymodel->commute)) ? implode(',', $propertymodel->commute) : NULL;
...
//
                                                        ...
                                                        $transaction1->commit();
                                                        return array('status' => 0, 'message' => 'Verification process is still in Pending.Please try after verification done from MSC');
                                                        exit;
                        ...
                        case "Booking";
                            if ($todolist['user_id'] == $user_id) {
                                ...
                                if ($todolist['landlord']['identity_status'] == 'Verified') {
                                    ...
                                    if ($todolist['landlord']['document_back'] != '') {
            ...
            if (!empty($_POST) && isset($_POST['document']) && $_POST['document'] != '' && isset($_POST['selfie']) && $_POST['selfie'] != '' && isset($_POST['request_id']) && $_POST['request_id'] != '' && isset($_POST['type']) && $_POST['type'] != '') {
                ...
                if ($type == 'N') {
                    ...
                    if ($journeyid != '') {
                        $document = $_POST['document'];
                        $document_back = (isset($_POST['document_back']) && $_POST['document_back'] != '') ? $_POST['document_back'] : '';
                        $document_no = (isset($_POST['document_no']) && $_POST['document_no'] != '') ? $_POST['document_no'] : '';
                                        ...
                                        if (!empty($getscorecardresult)) {
                                            if ($getscorecardresult->status == 'success') {
                                                ...
                                                if ($status == 'clear') {
                                                    ...
                                                    $usermodel->document_no = $document_no;
    ...
    public function actionMsctrustgate()
    {
        if (!empty($_POST) && isset($_POST['full_name']) && $_POST['full_name'] != '' && isset($_POST['identification_no']) && $_POST['identification_no'] != '' && isset($_POST['document']) && $_POST['document'] != '' && isset($_POST['type']) && $_POST['type'] != '' && isset($_POST['request_id']) && $_POST['request_id'] != '') {
            ...
            if (!empty($certificateexist)) {
                ...
                $mscmodel->created_at = date('Y-m-d H:i:s');
                $mscmodel->save(false);
                return array('status' => 1, 'message' => 'Your MSC certificate already exist in our system.So we are moving ahead to proceed signing');
                exit;
